...
-(id) initWithSuperView:(UIView *) view Title:(NSString *) title{
    return [super initWithSuperView:view Title:title LeftButton:@"browser_icon_close"];
}
-(void) onShare{
...
-(void) setExtraViewInits{
    UIImage *imgIconRefresh=[UIImage imageNamed:@"browser_icon_refresh"];
    UIImage *imgIconBack=[UIImage imageNamed:@"browser_icon_back"];
//    UIImage *imgIconBackDisabled=[UIImage imageNamed:@"browser_icon_back_disabled"];
    UIImage *imgIconForward=[UIImage imageNamed:@"browser_icon_forward"];
//    UIImage *imgIconForwardDisabled=[UIImage imageNamed:@"browser_icon_forward_disabled"];
    UIImage *imgIconShare=[UIImage imageNamed:@"browser_share"];
...
-(void) onClick:(UIButton *) button{
    int index=[curButtons indexOfObject:button];
    switch (index) {
        case 0:
            [webView goBack];
            break;
...
//    NSLog(@"web didFailLoadWithError %@",error);
    if ([error code] != NSURLErrorCancelled) {
        ...
        [[LEMainViewController instance] setMessageText:error.localizedDescription];
    }
}
