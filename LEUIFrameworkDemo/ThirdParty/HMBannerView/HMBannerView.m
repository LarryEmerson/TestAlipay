...
@implementation HMBannerViewImageView
-(id) init{
...
- (id)initWithFrame:(CGRect)frame scrollDirection:(BannerViewScrollDirection)direction images:(NSArray *)images  ImageViewClassName:(NSString *) className
{
    ...
    if(!curImageViewClassName){
        curImageViewClassName=@"HMBannerViewImageView";
    }
    ...
    if(self)
    {
        ...
        {
            HMBannerViewImageView *imageView = [[curImageViewClassName getInstanceFromClassName] performSelector:NSSelectorFromString(@"init")];
...
//    if(imagesArray.count==0)return;
    ...
    {
        HMBannerViewImageView *imageView = (HMBannerViewImageView *)[scrollView viewWithTag:Banner_StartTag+i];
        NSDictionary *dic = [curimageUrls objectAtIndex:i];
        NSString *url = [dic objectForKey:@"img_url"];
        if (imageView && [imageView isKindOfClass:[HMBannerViewImageView class]] && [url isNotEmpty])
        {
